import { cn } from "../../../Components/enterprise/utils";

const tones: Record<string, string> = {
  New: "border-cyan-400/30 bg-cyan-400/10 text-cyan-200",
  Qualified: "border-emerald-400/30 bg-emerald-400/10 text-emerald-200",
  Proposal: "border-blue-400/30 bg-blue-400/10 text-blue-200",
  Negotiation: "border-violet-400/30 bg-violet-400/10 text-violet-200",
  Active: "border-emerald-400/30 bg-emerald-400/10 text-emerald-200",
  Open: "border-amber-400/30 bg-amber-400/10 text-amber-200",
  Review: "border-amber-400/30 bg-amber-400/10 text-amber-200",
  Nurture: "border-slate-600 bg-slate-800 text-slate-300",
  Primary: "border-cyan-400/30 bg-cyan-400/10 text-cyan-200",
  Scheduled: "border-cyan-400/30 bg-cyan-400/10 text-cyan-200",
  Planned: "border-violet-400/30 bg-violet-400/10 text-violet-200",
  Pending: "border-slate-600 bg-slate-800 text-slate-300",
  High: "border-rose-400/30 bg-rose-400/10 text-rose-200",
  Medium: "border-amber-400/30 bg-amber-400/10 text-amber-200",
  Pinned: "border-cyan-400/30 bg-cyan-400/10 text-cyan-200",
  Draft: "border-slate-600 bg-slate-800 text-slate-300",
  Shared: "border-emerald-400/30 bg-emerald-400/10 text-emerald-200",
  Stored: "border-violet-400/30 bg-violet-400/10 text-violet-200",
  Logged: "border-emerald-400/30 bg-emerald-400/10 text-emerald-200",
  Sent: "border-cyan-400/30 bg-cyan-400/10 text-cyan-200",
};

export const CrmStatusBadge = ({ status }: { status: string }) => (
  <span className={cn("inline-flex h-7 items-center rounded-full border px-2.5 text-xs font-semibold", tones[status] || "border-slate-700 bg-slate-900 text-slate-300")}>{status}</span>
);
