...
const DealerCreate = () => {
  ...
  const onSubmit = async (values) => {
    try {
      await dealersService.createDealer(values);
      ...
      <div className="bg-white rounded-xl border p-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input {...register('name', { required: true })} className="w-full px-4 py-2 rounded-lg border" />
            {errors.name && <span className="text-red-600 text-sm">Required</span>}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('city')} className="w-full px-4 py-2 rounded-lg border" />
            </div>

            <div>
              <label className="block text-sm font-medium">State</label>
              <input {...register('state')} className="w-full px-4 py-2 rounded-lg border" />
            </div>
          </div>

          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => navigate('/dealers')} className="px-4 py-2 rounded-lg bg-[#F4F0FB] text-[#5B3FD6]">Cancel</button>
            <button type="submit" disabled={isSubmitting} className="px-4 py-2 rounded-lg bg-[#5B3FD6] text-white">Create</button>
          </div>
        </form>
      </div>
    </div>
